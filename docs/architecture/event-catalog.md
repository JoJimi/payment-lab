...
| DLQ 토픽 이름 | `<토픽>-dlt` | `DeadLetterPublishingRecoverer`(spring-kafka 4.1.1) 기본값 — 흔히 알려진 `.DLT` 접미사가 아니라 소문자 하이픈이다(2.16, `common-kafka` 실측). 재시도(500ms 3회)를 다 소진한 메시지가 여기 쌓인다 |
