...
### 8. 방어 로직 없는 버전과 비교 — 커넥션이 아니라 Kafka 리스너 스레드가 고갈된다 (3.9)

**재현 방법**: `main`을 커밋하지 않는 임시 로컬 브랜치에서 두 곳만 바꿔 3.1~3.6 이전
상태를 재현했다 — 이 변경은 어디에도 push되지 않았고 실험 후 브랜치째 버렸다.
1. `ResilientMockPgGateway.requestPayment` 맨 앞에서 Retry/CircuitBreaker/TimeLimiter/
   Bulkhead를 전부 건너뛰고 `mockPgClient.requestPayment(...)`를 호출자 스레드에서 직접,
   동기로 호출하도록 되돌렸다.
2. `resilience4j.timelimiter.instances.mockPg.timeout-duration`을 `3s`에서 `30s`로
   늘렸다 — `MockPgClient`가 이 값을 자기 HTTP read timeout으로도 그대로 읽어 쓰므로,
   그대로 두면 "방어 로직 없는 버전"인데도 클라이언트가 3초 만에 알아서 끊어버려 5초
   지연을 온전히 겪지 못한다. 코드에 남아 있던 이 결합은 그 자체로 흥미로운 사실이다 —
   `ResilientMockPgGateway`를 걷어내는 것만으로는 "완전 무방비"가 재현되지 않는다.

**부하**: Mock PG에 `delayMs=5000`(강제 다운이 아니라 5초 지연)을 걸고, order-service에
주문 10개를 거의 동시에 보냈다. order-service의 `POST /api/orders` 응답은 10개 전부
즉시 201 — order-service는 결제 처리와 같은 트랜잭션이 아니므로(2.12) 전혀 영향받지
않는다. 병목은 payment-service의 `payment.requested` 리스너 쪽에서만 나타난다.

**결과 — `payments` 테이블로 측정**: payment-service의 Kafka 리스너는 기본 설정상
concurrency=1(명시적으로 늘린 적이 없다, `application-dev.yml` 확인)이다. `requested_at`
(리스너가 메시지를 집어 든 시각)과 `approved_at`(처리 완료 시각)을 비교하면:

| order_id | requested_at | approved_at | elapsed |
|---|---|---|---|
| 802 | 07:32.166 | 07:38.109 | 5.94s |
| 803 | 07:40.078 | 07:45.644 | 5.57s |
| 804 | 07:45.934 | 07:51.155 | 5.22s |
| 805 | 07:51.567 | 07:57.261 | 5.69s |
| 806 | 08:00.903 | 08:08.076 | 7.17s |
| 807 | 08:19.551 | 08:28.057 | 8.51s |
| 808 | 08:30.482 | 08:39.004 | 8.52s |
| 809 | 08:46.140 | 08:52.370 | 6.23s |
| 810 | 08:52.678 | 08:58.270 | 5.59s |
| 811 | 09:00.465 | 09:05.862 | 5.40s |

(시각은 2026-09-25 15시대 UTC, 분:초.밀리초만 표기) 10건 모두 5~8.5초가 걸렸고, **다음 건의
`requested_at`은 항상 이전 건의 `approved_at` 이후에만 나타난다** — 거의 동시에 보낸
주문 10개가 리스너 스레드 하나에 완전히 직렬화됐다는 뜻이다. 첫 건이 리스너에 들어온
07:32.166부터 마지막 건이 승인된 09:05.862까지 총 93.7초 — 평균 9.37초/건(5초 PG
지연 + Kafka 재폴링 오버헤드)이 걸렸다. 같은 테이블의 직전 정상 실행분(802 이전
행들, 방어 로직이 살아있고 PG 지연도 없던 상태)은 `elapsed`가 0.05~1.1초였다 — 두
자릿수 차이다.

**왜 "커넥션 풀"이 아니라 "Kafka 리스너 스레드"인가**: 로드맵 3.9는 "커넥션 풀 고갈"이라고
표현하지만, 이 아키텍처에서 실제로 고갈되는 자원은 DB 커넥션이 아니다 — `PaymentService`는
애초에 Mock PG 호출을 `@Transactional` 밖에서 하도록 설계돼 있어(3.6 이전에 이미 확정된
원칙, `doRequestPayment` Javadoc) DB 커넥션은 방어 로직 유무와 무관하게 원래도 보호된다.
방어 로직이 없을 때 실제로 막히는 건 **Kafka 컨슈머 스레드 자체**다 — Bulkhead 없이는
PG 호출이 리스너 스레드에서 직접, 완전 동기로 실행되고, 이 리스너의 concurrency가
기본값 1이라 그 스레드가 막히는 순간 `payment.requested` 토픽 전체가 멈춘다. 방어
로직이 있는 버전이었다면 무엇이 달랐을지도 코드로 설명할 수 있다 — 실제로 재실행해
측정하지는 않았지만, 이미 배선된 설정값(1절, `sliding-window-size: 10`,
`minimum-number-of-calls: 5`, `failure-rate-threshold: 50`, 3.4의
`timeout-duration: 3s`)으로 추론하면:
- TimeLimiter가 매 시도를 3초에서 끊으므로, 방어 로직이 있었다면 5초 지연이 그대로
  리스너 스레드를 막는 일 자체가 없다 — 최악이어도 호출당 3초.
- 처음 5건(minimum-number-of-calls) 정도가 실제로 시간 초과를 겪고 나면 실패율이
  50%를 넘어 서킷이 OPEN된다 — 그 뒤로는 `CallNotPermittedException`이 즉시 던져져
  리스너가 사실상 거의 대기 없이 다음 메시지로 넘어간다.
- 즉 방어 로직의 핵심 효과는 "여러 요청을 동시에 처리하게 해준다"가 아니라(리스너
  스레드는 어차피 하나뿐이다) **장애가 지속돼도 백로그가 무한정 쌓이지 않고 어느
  지점에서 손절하게 만든다**는 데 있다 — 위 표의 93.7초짜리 완전 직렬 지연 대신,
  대략 5×3초(TimeLimiter 상한) + 나머지는 즉시 실패 ≈ 15~20초 안에 10건을 모두
  UNKNOWN으로 손절하고 정상 트래픽을 다시 받을 준비가 됐을 것이다.

**결론**: 3.9가 원래 가정한 "PG가 5초 지연될 때 방어 없으면 커넥션 풀이 고갈되어 전체가
멈춘다"는 현상은 이 아키텍처에서 커넥션 풀이 아니라 Kafka 리스너 스레드(그리고 그로 인한
토픽 전체 정체) 형태로 나타난다는 걸 실측으로 확인했다 — 결과 자체(요청마다 그대로
지연을 다 겪고, 완전히 직렬화되며, 장애가 계속되면 백로그가 무한정 자란다)는 로드맵이
말하는 "전체가 멈추는" 현상과 본질적으로 같다. 이 실험 코드(`ResilientMockPgGateway`
우회, `timeout-duration: 30s`)는 임시 로컬 브랜치에서만 존재했고 어디에도 병합되지
않았다.
